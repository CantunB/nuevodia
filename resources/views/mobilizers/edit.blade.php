    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form id="form" method="POST" action="{{ route('Mobilizers.update', $movilizador -> id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group mb-3">
                                        ...
                                        <select name="lider_id" class="lider form-control" id="lider" required>

                                            <option selected disabled value>SELECCIONA UN LIDER</option>
                                            @foreach($lideres as $lider)
                                                <option value="{{ $lider->getInfo->id  }}"
                                                @if($lider_id->getInfoLider->id == $lider->getInfo->id )
                                                    selected
                                                @endif
                                                >{{ $lider->getInfo->nombre .' '. $lider->getInfo->paterno .' '. $lider->getInfo->materno}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group mb-3">
                                        <label for="example-select">USUARIOS</label>
                                        <select name="user_id" class="lider form-control" id="lider" required>

                                            <option selected disabled value>SELECCIONA UN USUARIO</option>
                                            @foreach($usuarios as $usu)
                                                <option value="{{ $usu->id }}"
                                                @if($usu->id == $movilizador->user_id )
                                                    selected
                                                @endif
                                                >{{ $usu->nombre}}</option>
                                            @endforeach
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group mb-3">
                                        ...
                                        <input required type="text" name="clave_elector" value="{{ $movilizador->clave_elector }}" class="form-control" autocomplete="off">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group mb-3">
                                        <label for="simpleinput">NOMBRE</label>
                                        <input required type="text" name="nombre" value="{{ $movilizador->nombre }}" class="form-control" autocomplete="off">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group mb-3">
                            ...
                            <div class="modal-footer">
                                <a href="{{ route('Mobilizers.index') }}" class="btn btn-light" data-dismiss="modal">CERRAR</a>
